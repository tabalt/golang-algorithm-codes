...
func Test_Bucket_Sort_All(t *testing.T) {

	for idx, inputs := range bucketSortInputCases {
		...
		if !utils.SliceEqual(arr, inputs[1]) {
			t.Errorf("BucketSort error, index: %d, result: %v, expect: %v", idx, arr, inputs[1])
		}
